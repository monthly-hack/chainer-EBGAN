...
class discriminator(chainer.Chain):
    ...
    def __init__(self, n_units, n_canvas):
        super(discriminator, self).__init__(
            ...
            fc5=L.Linear(n_units, 1, wscale=0.002, bias=0),
        )
...
class gan_updater(training.StandardUpdater):
    ...
    def __init__(self, iterator, discriminator, generator,
        ...
        self.iteration = 0
        self._xp = xp
        self.ones = chainer.Variable(self._xp.ones(
                                     (batchsize, 1), dtype=np.int32))
        self.zeros = chainer.Variable(self._xp.zeros(
                                      (batchsize, 1), dtype=np.int32))
        self.sum_loss_dis = 0
        self.sum_loss_gen = 0
